...
import PanelPublicar from '../components/bolsa/PanelPublicar.jsx'
import TarjetaPublicacion from '../components/bolsa/TarjetaPublicacion.jsx'
import {
  CATEGORIAS,
  CATEGORIAS_EXTRA,
  HERO_BOLSA,
...
const CLAVE_FAVORITOS = 'somos-bolsa-favoritos'
...
function useFavoritos() {
  const [ids, setIds] = useState(() => {
    try {
      return new Set(JSON.parse(localStorage.getItem(CLAVE_FAVORITOS) || '[]'))
    } catch {
      return new Set()
    }
  })
  const alternar = useCallback((id) => {
    setIds((prev) => {
      ...
      localStorage.setItem(CLAVE_FAVORITOS, JSON.stringify([...siguiente]))
      return siguiente
    })
  }, [])
  return [ids, alternar]
...
export default function BolsaPage() {
  useTitulo('Bolsa de trabajo y servicios locales · Somos Chapulhuacanito')
  ...
  const categorias = listaParam(params.get('cat'))
  const zona = params.get('zona') || ''
  const tipos = listaParam(params.get('tipo'))
  ...
  }, [q, categorias, zona, tipos, orden, soloDestacados, setParams])
  ...
  useEffect(() => {
    if (categorias.some((id) => CATEGORIAS_EXTRA.some((c) => c.id === id))) setMasCategorias(true)
  }, [categorias])
  ...
  const destacados = soloDestacados ? publicaciones : publicaciones.filter((p) => p.destacado)
  const recientesBase = soloDestacados ? [] : publicaciones.filter((p) => !p.destacado)
  ...
  function alternarCategoria(id) {
    const siguiente = categorias.includes(id) ? categorias.filter((c) => c !== id) : [...categorias, id]
    actualizar({ cat: siguiente.join(','), destacados: '' })
  }

  function alternarTipo(id) {
    const siguiente = tipos.includes(id) ? tipos.filter((t) => t !== id) : [...tipos, id]
  ...
  function buscarEmpleo() {
    actualizar({ tipo: 'empleo', destacados: '', cat: '' })
  ...
  return (
    <div className="pb-8">
      <section className="relative isolate overflow-hidden" aria-labelledby="titulo-bolsa">
        <img src={HERO_BOLSA} alt="Ilustración de la iglesia y las casas de Chapulhuacanito entre cerros de la Huasteca" className="absolute inset-0 -z-20 h-full w-full object-cover object-[70%_40%]" />
        <div className="absolute inset-0 -z-10 bg-gradient-to-r from-black/75 via-black/40 via-40% to-transparent" />
        <img src={publico('assets/img/cempasuchil-izq.svg')} alt="" aria-hidden="true" className="pointer-events-none absolute -bottom-6 -left-10 -z-10 w-28 sm:w-40" />
        <img src={publico('assets/img/cempasuchil-der.svg')} alt="" aria-hidden="true" className="pointer-events-none absolute -right-8 bottom-6 -z-10 hidden w-36 md:block" />
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8 lg:py-20">
          <div className="max-w-3xl">
            ...
            <div className="mt-3 flex flex-wrap items-start gap-4">
              ...
              <p className="inline-flex items-center rounded-full bg-white px-4 py-2 text-sm font-bold text-naranja shadow-tarjeta lg:mt-3 lg:-rotate-2">Apoyemos el talento local</p>
            </div>
            ...
            >
              ...
              <div className="flex items-center rounded-full bg-white p-1.5 pl-4 shadow-suave ring-1 ring-black/5 focus-within:ring-2 focus-within:ring-naranja">
                ...
                />
                <button type="submit" className="shrink-0 rounded-full bg-naranja px-4 py-2.5 text-sm font-semibold text-white hover:bg-naranja-600 sm:px-5">Buscar</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="relative z-10 -mt-6 rounded-3xl bg-white p-3 shadow-suave sm:p-4">
          <div className="sin-scroll flex gap-2 overflow-x-auto pb-1" role="toolbar" aria-label="Categorías">
            <button type="button" aria-pressed={categorias.length === 0 && !soloDestacados} onClick={() => actualizar({ cat: '', destacados: '' })} className={`inline-flex min-h-11 shrink-0 items-center gap-2 rounded-full px-4 text-sm font-semibold ${categorias.length === 0 && !soloDestacados ? 'bg-naranja text-white' : 'bg-crema text-tinta'}`}>
              <Icono id="i-cuadros" className="icono h-4 w-4" />Todos
            </button>
            {CATEGORIAS.map((cat) => {
              const activo = categorias.length === 1 && categorias[0] === cat.id
              return (
                <button key={cat.id} type="button" aria-pressed={activo || categorias.includes(cat.id)} onClick={() => actualizar({ cat: activo ? '' : cat.id, destacados: '' })} className={`inline-flex min-h-11 shrink-0 items-center gap-2 rounded-full px-4 text-sm font-semibold ${activo ? `${cat.tono} text-white` : 'bg-crema text-tinta'}`}>
                  <Icono id={cat.icono} className="icono h-4 w-4" />{cat.nombre}
                </button>
              )
            })}
            <button type="button" aria-expanded={masCategorias} onClick={() => setMasCategorias((v) => !v)} className="inline-flex min-h-11 shrink-0 items-center gap-2 rounded-full border border-naranja/40 bg-white px-4 text-sm font-semibold text-naranja">
              <Icono id="i-mas" className="icono h-4 w-4" />Más categorías
            </button>
          </div>
          {masCategorias && (
            <div className="mt-2 flex flex-wrap gap-2">
              {CATEGORIAS_EXTRA.map((cat) => {
                ...
                return (
                  <button key={cat.id} type="button" aria-pressed={activo} onClick={() => alternarCategoria(cat.id)} className={`inline-flex min-h-11 items-center gap-2 rounded-full px-4 text-sm font-semibold ${activo ? `${cat.tono} text-white` : 'bg-crema text-tinta'}`}>
                    <Icono id={cat.icono} className="icono h-4 w-4" />{cat.nombre}
                  </button>
                )
              })}
        ...
        <div className="mt-6 lg:grid lg:grid-cols-[17.5rem_minmax(0,1fr)] lg:items-start lg:gap-8">
          <div className="lg:sticky lg:top-24">
            <button type="button" className="mb-3 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-full bg-white px-4 font-semibold shadow-tarjeta lg:hidden" aria-expanded={filtrosAbiertos} aria-controls="filtros-bolsa" onClick={() => setFiltrosAbiertos((v) => !v)}>
              <Icono id="i-filtro" className="icono h-5 w-5" />
            ...
            <aside id="filtros-bolsa" className={`${filtrosAbiertos ? 'block' : 'hidden'} rounded-3xl bg-white p-4 shadow-tarjeta lg:block lg:p-5`} aria-label="Filtros de la bolsa">
              <div className="mb-3 flex items-center justify-between gap-3">
                ...
                <button type="button" onClick={limpiar} className="min-h-11 text-sm font-semibold text-naranja hover:text-naranja-700">Limpiar filtros</button>
              </div>
              ...
              <div className="mt-2 flex items-center rounded-xl border border-crema-200 bg-crema px-3">
                <Icono id="i-buscar" className="icono h-4 w-4 text-tinta/50" />
              ...
              <select
                ...
                value={categorias.length === 1 ? categorias[0] : ''}
                onChange={(evento) => actualizar({ cat: evento.target.value, destacados: '' })}
              >
                  ...
                  {TODAS_CATEGORIAS.map((cat) => (
                    <li key={cat.id}>
                      <label className="flex min-h-11 cursor-pointer items-center gap-3 rounded-xl px-1 text-sm hover:bg-crema">
                        <input type="checkbox" className="h-5 w-5 accent-[#f26b1d]" checked={categorias.includes(cat.id)} onChange={() => alternarCategoria(cat.id)} />
                        {cat.nombre}
                      </label>
                    </li>
                  ))}
                  ...
                  {TIPOS.map((tipo) => (
                    <li key={tipo.id}>
                      <label className="flex min-h-11 cursor-pointer items-center gap-3 text-sm">
                        <input type="checkbox" className="h-5 w-5 accent-[#f26b1d]" checked={tipos.includes(tipo.id)} onChange={() => alternarTipo(tipo.id)} />
                        {tipo.nombre}
                      </label>
                    </li>
                  ))}
          ...
          <div id="resultados" className="mt-6 scroll-mt-28 lg:mt-0">
            ...
            {publicaciones.length === 0 ? (
              <div className="mt-4 rounded-3xl bg-white p-8 text-center shadow-tarjeta">
                <h2 className="text-xl font-bold">No hay publicaciones con esos filtros</h2>
                <p className="mt-2 text-tinta/70">Prueba con otra palabra, otra zona o limpia los filtros para ver el talento de la comunidad.</p>
                <button type="button" onClick={limpiar} className="mt-5 inline-flex min-h-11 items-center rounded-full bg-naranja px-5 font-semibold text-white hover:bg-naranja-600">Limpiar filtros</button>
              </div>
            ) : (
              <>
                {destacados.length > 0 && (
                  <section className="mt-4" aria-labelledby="titulo-destacados">
                    <div className="mb-3 flex items-end justify-between gap-3">
                      ...
                      <button type="button" onClick={() => actualizar({ destacados: '1', cat: '', tipo: '', zona: '', q: '' })} className="inline-flex min-h-11 shrink-0 items-center gap-1 text-sm font-semibold text-naranja">
                        Ver todos <Icono id="i-chevron" className="icono h-4 w-4" />
                      </button>
                    </div>
                    <div className="sin-scroll -mx-4 flex snap-x gap-4 overflow-x-auto px-4 pb-2 lg:mx-0 lg:grid lg:grid-cols-2 lg:overflow-visible lg:px-0 xl:grid-cols-4">
                      {destacados.slice(0, soloDestacados ? destacados.length : 4).map((p) => (
                        <TarjetaPublicacion key={p.id} publicacion={p} destacada favorito={favoritos.has(p.id)} onFavorito={alternarFavorito} />
                      ))}
                    </div>
                  </section>
                )}
                  ...
                  <section className="mt-10" aria-labelledby="titulo-recientes">
                    <div className="mb-3 flex items-end justify-between gap-3">
                      ...
                      {recientesBase.length > 8 && !verTodasRecientes ? (
                        <button type="button" onClick={() => setVerTodasRecientes(true)} className="inline-flex min-h-11 items-center gap-1 text-sm font-semibold text-naranja">
                          Ver todos <Icono id="i-chevron" className="icono h-4 w-4" />
                        </button>
                      ) : null}
                    </div>
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
                      {recientes.map((p) => (
                        <TarjetaPublicacion key={p.id} publicacion={p} favorito={favoritos.has(p.id)} onFavorito={alternarFavorito} />
                      ))}
                    </div>
                  </section>
                )}
              ...
              <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div className="max-w-xl">
                  <p className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-naranja-50 text-naranja" aria-hidden="true">
                    <Icono id="i-megafono" className="icono h-6 w-6" />
                  </p>
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                  <button type="button" onClick={() => abrirPublicar('servicio')} className="inline-flex min-h-12 items-center justify-center rounded-full bg-naranja px-5 font-semibold text-white shadow-md shadow-naranja/30 hover:bg-naranja-600">Publicar mi servicio</button>
                  <button type="button" onClick={buscarEmpleo} className="inline-flex min-h-12 items-center justify-center rounded-full bg-white px-5 font-semibold text-naranja ring-1 ring-naranja/40 hover:bg-naranja-50">Buscar empleo</button>
                </div>
              </div>
              ...
              <ul className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-5">
                {[
                  ...
                  <li key={texto} className="rounded-2xl bg-crema p-4">
                    <span className="grid h-11 w-11 place-items-center rounded-2xl bg-white text-naranja shadow-sm"><Icono id={icono} className="icono h-5 w-5" /></span>
